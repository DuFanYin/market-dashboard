...
- Fear & Greed: score, rating, segmented bar, historical snapshots, and component signals
- AHR999: computed client view with current zone coloring
- Portfolio dashboard: password-protected view with positions table, Greeks, donut breakdown, and refresh-on-demand
...
```
data/
└── positions.yaml            # Portfolio source positions (also generated by ibkr script)
src/
├── app
│   ├── api
│   │   ├── market
│   │   │   └── route.ts
│   │   └── portfolio
│   │       └── route.ts      # Serverless portfolio API (basic auth + Tradier integration)
│   ├── dashboard
│   │   ├── page.module.css
│   │   └── page.tsx
│   ├── portfolio
│   │   ├── page.module.css
│   │   └── page.tsx          # Client-side portfolio UI (fetches /api/portfolio)
│   ├── globals.css
│   ├── layout.tsx
│   └── page.tsx
├── components
...
- Portfolio dashboard refreshes on-demand via the “Refresh” button (no background polling)
...
- Generate portfolio snapshot from IBKR: `python position/ibkr.py` (writes to `data/positions.yaml`)

## Environment Variables
- `.env` at the project root is loaded by Next.js. Set:
  - `TRADIER_TOKEN` – Tradier API bearer token
  - `PORTFOLIO_PASSWORD` – password required for `/portfolio` and `/api/portfolio`
- Optional:
  - `TRADIER_BASE_URL` – override Tradier base URL
  - IBKR connection vars (`IB_HOST`, `IB_PORT`, `IB_CLIENT_ID`) for `position/ibkr.py`
